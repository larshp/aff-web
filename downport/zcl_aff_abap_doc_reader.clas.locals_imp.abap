...
*
    ...
    DATA l_line_code_condensed LIKE l_line_code.
    DATA l_hier LIKE LINE OF me->hierarchy_nodes.
...
*             Check whether this is a TYPES BEGIN OF MESH my_mesh statement.
        ...
        IF relevant_token1-str = 'TYPES' OR relevant_token1-str = 'DATA' OR relevant_token1-str = 'CLASS-DATA' OR relevant_token1-str = 'CONSTANTS'.
          ...
          LOOP AT me->hierarchy_nodes INTO l_hier.
  ...
  METHOD build_hierarchy_nodes.
    ...
    DATA temp3 LIKE LINE OF tab_tokens.
    DATA temp4 LIKE sy-tabix.
    DATA temp1 LIKE LINE OF tab_tokens.
    DATA temp2 LIKE sy-tabix.
    DATA temp8 LIKE LINE OF tab_tokens.
    DATA temp9 LIKE sy-tabix.
    DATA temp10 LIKE LINE OF tab_tokens.
    DATA temp11 LIKE sy-tabix.
    DATA l_lines TYPE i.
    DATA temp5 LIKE LINE OF parents.
    ...
    LOOP AT tab_statements ASSIGNING <fs_stmnt>  WHERE ( type <> 'P' AND type <> 'S' AND type <> 'G' ). " no comments
      ...
      temp4 = sy-tabix.
      READ TABLE tab_tokens INDEX <fs_stmnt>-from INTO temp3.
      sy-tabix = temp4.
      IF sy-subrc <> 0.
      ...
      temp2 = sy-tabix.
      READ TABLE tab_tokens INDEX <fs_stmnt>-from INTO temp1.
      sy-tabix = temp2.
      IF sy-subrc <> 0.
        RAISE EXCEPTION TYPE cx_sy_itab_line_not_found.
      ENDIF.
      ...
      temp9 = sy-tabix.
      READ TABLE tab_tokens INDEX <fs_stmnt>-from INTO temp8.
      sy-tabix = temp9.
      IF sy-subrc <> 0.
        RAISE EXCEPTION TYPE cx_sy_itab_line_not_found.
      ENDIF.
      ...
      temp11 = sy-tabix.
      READ TABLE tab_tokens INDEX <fs_stmnt>-from INTO temp10.
      sy-tabix = temp11.
      IF sy-subrc <> 0.
        RAISE EXCEPTION TYPE cx_sy_itab_line_not_found.
      ENDIF.
      IF NOT ( temp3-str = 'TYPES' OR temp1-str = 'DATA' OR
               temp8-str = 'CLASS-DATA' OR temp10-str = 'CONSTANTS' ).
        CONTINUE.
      ENDIF.
      ...
      LOOP AT tab_tokens ASSIGNING <fs_tok> FROM <fs_stmnt>-from TO <fs_stmnt>-to.
        ...
        IF <fs_tok_next1> IS ASSIGNED AND <fs_tok_next2> IS ASSIGNED AND <fs_tok_next3> IS ASSIGNED.
          ...
          ELSEIF ( <fs_tok_next1>-str = 'END' AND <fs_tok_next2>-str = 'OF' ).
            ...
            l_lines = lines( parents ).
            DELETE parents INDEX l_lines.


            temp6 = sy-tabix.
            READ TABLE parents INDEX l_depth INTO temp5.
            sy-tabix = temp6.
            IF sy-subrc <> 0.
              RAISE EXCEPTION TYPE cx_sy_itab_line_not_found.
            ENDIF.
            l_node-parent_node = temp5.
