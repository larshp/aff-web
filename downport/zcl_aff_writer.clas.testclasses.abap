...
CLASS ltcl_type_writer IMPLEMENTATION.
  ...
  METHOD get_output.
    ...
    CLEAR temp41.
    APPEND `line1` TO temp41.
    APPEND `line2` TO temp41.
  ...
  METHOD get_type_info_string_like.
    ...
    DATA temp46 TYPE char1.
    DATA temp47 TYPE xstring.
    DATA temp48 TYPE numc4.
    ...
    CLEAR temp48.
    cl_abap_unit_assert=>assert_equals( exp = zif_aff_writer=>type_info-string act = cut->get_json_type_from_description( get_element_description( temp48 ) ) ).
  ENDMETHOD.

  METHOD get_type_info_boolean1.
    DATA temp49 TYPE abap_boolean.
    CLEAR temp49.
  ...
  METHOD get_type_info_boolean3.
    DATA temp51 TYPE flag.
    CLEAR temp51.
    cl_abap_unit_assert=>assert_equals( exp = zif_aff_writer=>type_info-boolean act = cut->get_json_type_from_description( get_element_description( temp51 ) ) ).
  ENDMETHOD.

  METHOD get_type_info_numeric.
    DATA packed TYPE p.
    DATA temp52 TYPE f.
    DATA temp53 TYPE int1.
    DATA temp54 TYPE int2.
    DATA temp55 TYPE int8.
    DATA temp56 TYPE decfloat16.
    DATA temp57 TYPE decfloat34.
    ...
    CLEAR temp57.
    cl_abap_unit_assert=>assert_equals( exp = zif_aff_writer=>type_info-numeric act = cut->get_json_type_from_description( get_element_description( temp57 ) ) ).
  ENDMETHOD.

  METHOD get_type_info_date_time.
    DATA temp58 TYPE d.
    DATA temp59 TYPE t.
    DATA temp60 TYPE timestamp.
    DATA temp61 TYPE timestampl.
    DATA temp62 TYPE utclong.
    CLEAR temp58.
    ...
    CLEAR temp62.
    cl_abap_unit_assert=>assert_equals( exp = zif_aff_writer=>type_info-date_time act = cut->get_json_type_from_description( get_element_description( temp62 ) ) ).
  ENDMETHOD.

  METHOD stack_stores_operations.
    DATA table TYPE STANDARD TABLE OF tadir.
    DATA temp63 TYPE REF TO cl_abap_elemdescr.
    DATA temp64 TYPE tadir.
    DATA temp65 TYPE tadir.

    cl_abap_unit_assert=>assert_equals( exp = zif_aff_writer=>operation-initial act = cut->last_operation( ) ).


    CLEAR temp63.
    cut->zif_aff_writer~write_element( element_name = 'write_element' element_description = temp63 ).
    cl_abap_unit_assert=>assert_equals( exp = zif_aff_writer=>operation-write_element act = cut->last_operation( ) ).


    CLEAR temp64.
    cut->zif_aff_writer~open_node( node_name = 'open_structure' node_description = cl_abap_typedescr=>describe_by_data( temp64 ) ).
    cl_abap_unit_assert=>assert_equals( exp = zif_aff_writer=>operation-open_structure act = cut->last_operation( ) ).


    CLEAR temp65.
    cut->zif_aff_writer~close_node( node_name = 'close_structure' node_description = cl_abap_typedescr=>describe_by_data( temp65 ) ).
  ...
  METHOD get_element_description.
    ...
    temp66 ?= cl_abap_typedescr=>describe_by_data( data ).
    result = temp66.
  ENDMETHOD.

  METHOD append_to_previous_line.
    DATA temp67 LIKE LINE OF cut->content.
    DATA temp68 LIKE sy-tabix.
    DATA temp69 LIKE LINE OF cut->content.
    ...
    temp68 = sy-tabix.
    READ TABLE cut->content INDEX 1 INTO temp67.
    sy-tabix = temp68.
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE cx_sy_itab_line_not_found.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = '1+2=2+1' act = temp67 ).


    temp70 = sy-tabix.
    READ TABLE cut->content INDEX 2 INTO temp69.
    sy-tabix = temp70.
    IF sy-subrc <> 0.
      RAISE EXCEPTION TYPE cx_sy_itab_line_not_found.
    ENDIF.
    cl_abap_unit_assert=>assert_equals( exp = 'new line;' act = temp69 ).
  ENDMETHOD.
  ...
  METHOD call_reader_and_decode4.
    ...
    CLEAR abap_doc_exp.
    abap_doc_exp-title = 'Packed Number With Given Multiple'.
    abap_doc_exp-description = `Packed number with given multiple`.
    abap_doc_exp-exclusive_minimum = `0`.
    abap_doc_exp-maximum = `99999.90`.
    abap_doc_exp-multiple_of = `0.1`.
    cl_abap_unit_assert=>assert_equals( exp = abap_doc_exp act = abap_doc_act ).
  ENDMETHOD.
  ...
  METHOD delete_first_of_struc_stack.
    ...
    INSERT temp71 INTO cut->stack_of_structure INDEX 1.

    CLEAR temp72.
    temp72-name = `MY_STRUCTURE3`.
    temp72-absolute_name = `\CLASS=ZCL_AFF_TEST_TYPES\TYPE=MY_STRUCTURE3`.
    INSERT temp72 INTO exp_table INDEX 1.

    CLEAR temp73.
    temp73-name = `MY_NESTED_STRUCTURE`.
    temp73-absolute_name = `\CLASS=ZCL_AFF_TEST_TYPES\TYPE=MY_NESTED_STRUCTURE`.
  ...
  METHOD get_all_path_information.
    ...
    DATA temp75 TYPE zcl_aff_writer=>ty_structure_stack.
    ...
    CLEAR temp74.
    temp74-name = `LIST`.
    temp74-absolute_name = `\CLASS=ZCL_AFF_TEST_TYPES\TYPE=LIST`.
    INSERT temp74 INTO cut->stack_of_structure INDEX 1.

    CLEAR temp75.
    temp75-name = `LIST1`.
    temp75-absolute_name = `\TYPE=%_T00004S00000109O0000013408`.
    INSERT temp75 INTO cut->stack_of_structure INDEX 1.
  ...
  METHOD get_abap_doc_for_absolute_name.
    ...
    DATA temp76 TYPE abap_abstypename.
    ...
    temp76 = absolute_name.

    act_abap_doc = cut->get_abap_doc_for_absolute_name( absolute_name = temp76 ).
  ...
  METHOD get_struc_of_enum_values_cl.
    ...
    temp77 ?= cl_abap_structdescr=>describe_by_data( ref ).
    exp_structure = temp77.
  ...
  METHOD validate_default.
    ...
    DATA integer TYPE i.
    DATA temp103 TYPE descr_default_valid_tuple.
    DATA temp47 TYPE REF TO cl_abap_elemdescr.
    FIELD-SYMBOLS <entry> LIKE LINE OF table_of_tuples.
    DATA is_valid TYPE abap_boolean.
    CLEAR temp78.

    temp22 ?= cl_abap_elemdescr=>describe_by_data( date_time ).
    temp78-element_description = temp22.
    temp78-default = '9999-12-31T23:59:59.9999999'.
    temp78-is_valid = abap_false.
    ...
    temp23 ?= cl_abap_elemdescr=>describe_by_data( time ).
    temp79-element_description = temp23.
    temp79-default = '20'.
    ...
    temp24 ?= cl_abap_elemdescr=>describe_by_data( time ).
    temp80-element_description = temp24.
    temp80-default = '201500'.
    temp80-is_valid = abap_true.
    INSERT temp80 INTO TABLE table_of_tuples.

    CLEAR temp81.

    temp25 ?= cl_abap_elemdescr=>describe_by_data( time ).
    temp81-element_description = temp25.
    temp81-default = '20150045'.
    temp81-is_valid = abap_false.
    ...
    temp26 ?= cl_abap_elemdescr=>describe_by_data( date ).
    temp82-element_description = temp26.
    temp82-default = '20121201'.
    temp82-is_valid = abap_true.
    INSERT temp82 INTO TABLE table_of_tuples.

    CLEAR temp83.

    temp27 ?= cl_abap_elemdescr=>describe_by_data( date ).
    temp83-element_description = temp27.
    temp83-default = '201212214'.
    temp83-is_valid = abap_false.
    INSERT temp83 INTO TABLE table_of_tuples.

    CLEAR temp84.

    temp28 ?= cl_abap_elemdescr=>describe_by_data( date ).
    temp84-element_description = temp28.
    temp84-default = '201212'.
    ...
    temp29 ?= cl_abap_elemdescr=>describe_by_data( numeric ).
    temp85-element_description = temp29.
    temp85-default = '201'.
    ...
    temp30 ?= cl_abap_elemdescr=>describe_by_data( numeric ).
    temp86-element_description = temp30.
    temp86-default = '20'.
    temp86-is_valid = abap_true.
    INSERT temp86 INTO TABLE table_of_tuples.

    CLEAR temp87.

    temp31 ?= cl_abap_elemdescr=>describe_by_data( numeric ).
    temp87-element_description = temp31.
    temp87-default = '2021'.
    temp87-is_valid = abap_false.
    ...
    temp32 ?= cl_abap_elemdescr=>describe_by_data( float ).
    temp88-element_description = temp32.
    temp88-default = '14.5e12'.
    temp88-is_valid = abap_true.
    INSERT temp88 INTO TABLE table_of_tuples.

    CLEAR temp89.

    temp33 ?= cl_abap_elemdescr=>describe_by_data( float ).
    temp89-element_description = temp33.
    temp89-default = 'no float'.
    temp89-is_valid = abap_false.
    ...
    temp34 ?= cl_abap_elemdescr=>describe_by_data( decfloat ).
    temp90-element_description = temp34.
    temp90-default = '14.5e12'.
    temp90-is_valid = abap_true.
    INSERT temp90 INTO TABLE table_of_tuples.

    CLEAR temp91.

    temp35 ?= cl_abap_elemdescr=>describe_by_data( decfloat ).
    temp91-element_description = temp35.
    temp91-default = 'no_float'.
    ...
    temp36 ?= cl_abap_elemdescr=>describe_by_data( packed ).
    temp92-element_description = temp36.
    temp92-default = 'a'.
    ...
    temp37 ?= cl_abap_elemdescr=>describe_by_data( packed ).
    temp93-element_description = temp37.
    temp93-default = '4.534'.
    temp93-is_valid = abap_false.
    INSERT temp93 INTO TABLE table_of_tuples.

    CLEAR temp94.

    temp38 ?= cl_abap_elemdescr=>describe_by_data( packed ).
    temp94-element_description = temp38.
    temp94-default = '3.25'.
    ...
    temp39 ?= cl_abap_elemdescr=>describe_by_data( char ).
    temp95-element_description = temp39.
    temp95-default = 'a5b'.
    temp95-is_valid = abap_true.
    INSERT temp95 INTO TABLE table_of_tuples.

    CLEAR temp96.

    temp40 ?= cl_abap_elemdescr=>describe_by_data( char ).
    temp96-element_description = temp40.
    temp96-default = 'a5bca'.
    temp96-is_valid = abap_false.
    ...
    temp41 ?= cl_abap_elemdescr=>describe_by_data( boolean ).
    temp97-element_description = temp41.
    temp97-default = 'abap_true'.
    temp97-is_valid = abap_true.
    INSERT temp97 INTO TABLE table_of_tuples.

    CLEAR temp98.

    temp42 ?= cl_abap_elemdescr=>describe_by_data( boolean ).
    temp98-element_description = temp42.
    temp98-default = 'abap_false'.
    temp98-is_valid = abap_true.
    INSERT temp98 INTO TABLE table_of_tuples.

    CLEAR temp99.

    temp43 ?= cl_abap_elemdescr=>describe_by_data( boolean ).
    temp99-element_description = temp43.
    temp99-default = ''.
    temp99-is_valid = abap_true.
    INSERT temp99 INTO TABLE table_of_tuples.

    CLEAR temp100.

    temp44 ?= cl_abap_elemdescr=>describe_by_data( boolean ).
    temp100-element_description = temp44.
    temp100-default = 'a'.
    ...
    temp45 ?= cl_abap_elemdescr=>describe_by_data( integer1 ).
    temp101-element_description = temp45.
    temp101-default = '-5'.
    temp101-is_valid = abap_false.
    INSERT temp101 INTO TABLE table_of_tuples.

    CLEAR temp102.

    temp46 ?= cl_abap_elemdescr=>describe_by_data( integer1 ).
    temp102-element_description = temp46.
    temp102-default = '10'.
    ...
    temp47 ?= cl_abap_elemdescr=>describe_by_data( integer ).
    temp103-element_description = temp47.
    temp103-default = '-5'.
    temp103-is_valid = abap_true.
    INSERT temp103 INTO TABLE table_of_tuples.
  ...
  METHOD validate_source.
    DATA temp104 TYPE string_table.
    DATA temp48 TYPE REF TO zif_aff_log.
    CLEAR temp104.

    CLEAR temp48.
    cl_abap_unit_assert=>assert_true( cut->zif_aff_writer~validate( source = temp104 log = temp48 ) ).
  ENDMETHOD.

  METHOD handle_include.
    DATA data TYPE zcl_aff_test_types=>ty_include_type.
    DATA temp105 TYPE REF TO cl_abap_structdescr.
    temp105 ?= cl_abap_structdescr=>describe_by_data( data ).
    cut->zif_aff_writer~open_include( temp105 ).
