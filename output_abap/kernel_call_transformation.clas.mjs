    ...
    if (abap.compare.initial(kernel_call_transformation.mi_writer) === false) {
      ...
      for (const name in INPUT.source) {
        ...
        if (INPUT.result.constructor.name === "Table") {
          ...
          for await (const unique120 of abap.statements.loop(lt_rtab)) {
            ls_rtab.set(unique120);
          ...
          static async traverse_write_type(INPUT) {
            ...
            let unique121 = lo_type.get().type_kind;
            if (abap.compare.eq(unique121, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int) || abap.compare.eq(unique121, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int1) || abap.compare.eq(unique121, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int2) || abap.compare.eq(unique121, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_int8) || abap.compare.eq(unique121, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat) || abap.compare.eq(unique121, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat16) || abap.compare.eq(unique121, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_decfloat34)) {
