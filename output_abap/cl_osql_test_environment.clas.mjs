...
class cl_osql_test_environment {
  ...
  async validate() {
    ...
    for await (const unique331 of abap.statements.loop(this.mt_tables)) {
      lv_table.set(unique331);
      try {
  ...
  async initialize() {
    ...
    for await (const unique332 of abap.statements.loop(this.mt_tables)) {
      lv_table.set(unique332);
  ...
  async if_osql_test_environment$clear_doubles() {
    let lv_table = new abap.types.Character(30, {"qualifiedName":"abap_compname"});
    for await (const unique333 of abap.statements.loop(this.mt_tables)) {
      lv_table.set(unique333);
      lv_table.set(abap.builtin.to_lower({val: lv_table}));
