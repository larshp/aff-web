...
class $ui2$cl_json {
  ...
  static async serialize(INPUT) {
    ...
    let unique112 = lo_type.get().kind;
    if (abap.compare.eq(unique112, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
      let unique113 = lo_type.get().type_kind;
      if (abap.compare.eq(unique113, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_char) || abap.compare.eq(unique113, abap.Classes['CL_ABAP_TYPEDESCR'].typekind_string)) {
        r_json.set(abap.operators.concat(new abap.types.Character(1).set('"'),abap.operators.concat(data,new abap.types.Character(1).set('"'))));
      } else {
        r_json.set(data);
      }
    } else if (abap.compare.eq(unique112, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
      r_json.set(new abap.types.Character(1).set('['));
      abap.statements.assign({target: fs_tab_, source: data});
      for await (const unique114 of abap.statements.loop(fs_tab_)) {
        fs_any_.assign(unique114);
      ...
      r_json.set(abap.operators.concat(r_json,new abap.types.Character(1).set(']')));
    } else if (abap.compare.eq(unique112, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      abap.statements.assert(abap.compare.eq(new abap.types.Integer().set(1), new abap.types.Character(21).set('cl_json, unknown kind')));
    }
    return r_json;
  }
  ...
  static async deserialize(INPUT) {
    ...
    await this._deserialize({prefix: new abap.types.Character(0).set(''), pretty_name: pretty_name, data: data});
  }
  ...
  static async _deserialize(INPUT) {
    ...
    let fs_any_ = new abap.types.FieldSymbol(new abap.types.Character(4));
    lo_type.set((await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: data})));
    let unique116 = lo_type.get().kind;
    if (abap.compare.eq(unique116, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
      if (abap.compare.eq(lo_type.get().absolute_name, new abap.types.Character(30).set('\\TYPE-POOL=ABAP\\TYPE=ABAP_BOOL')) || abap.compare.eq(lo_type.get().absolute_name, new abap.types.Character(10).set('\\TYPE=FLAG'))) {
    ...
    } else if (abap.compare.eq(unique116, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
      ...
      for await (const unique117 of abap.statements.loop(lt_members)) {
        lv_member.set(unique117);
        abap.statements.createData(ref,{"likeLineOf": data});
        abap.statements.assign({target: fs_any_, source: (ref).getPointer()});
        await this._deserialize({prefix: abap.operators.concat(prefix,abap.operators.concat(new abap.types.Character(1).set('/'),lv_member)), pretty_name: pretty_name, data: fs_any_});
        abap.statements.insertInternal({data: fs_any_, table: data});
      }
    } else if (abap.compare.eq(unique116, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      await abap.statements.cast(lo_struct, lo_type);
      lt_components.set((await lo_struct.get().get_components()));
      for await (const unique118 of abap.statements.loop(lt_components)) {
        ...
        abap.statements.assign({component: ls_component.get().name, target: fs_any_, source: data});
        abap.statements.assert(abap.compare.eq(abap.builtin.sy.get().subrc, new abap.types.Integer().set(0)));
        let unique119 = pretty_name;
        if (abap.compare.eq(unique119, $ui2$cl_json.pretty_mode.get().camel_case)) {
        ...
        await this._deserialize({prefix: abap.operators.concat(prefix,abap.operators.concat(new abap.types.Character(1).set('/'),lv_name)), pretty_name: pretty_name, data: fs_any_});
      }
    } else {
      abap.statements.assert(abap.compare.eq(new abap.types.Integer().set(1), new abap.types.Character(21).set('cl_json, unknown kind')));
    }
