...
class lcl_json_parser {
  ...
  async traverse(INPUT) {
    ...
    if (abap.compare.eq(unique144, new abap.types.Character({length: 6}).set('object'))) {
      await this.traverse_object({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique144, new abap.types.Character({length: 5}).set('array'))) {
      await this.traverse_array({iv_json: iv_json, iv_key: iv_key});
    } else if (abap.compare.eq(unique144, new abap.types.Character({length: 6}).set('string')) || abap.compare.eq(unique144, new abap.types.Character({length: 7}).set('boolean')) || abap.compare.eq(unique144, new abap.types.Character({length: 6}).set('number')) || abap.compare.eq(unique144, new abap.types.Character({length: 4}).set('null'))) {
  ...
  async traverse_basic(INPUT) {
    ...
    if (abap.compare.eq(unique145, new abap.types.Character({length: 6}).set('string'))) {
      lv_type.set(new abap.types.Character({length: 3}).set('str'));
    } else if (abap.compare.eq(unique145, new abap.types.Character({length: 6}).set('number'))) {
      lv_type.set(new abap.types.Character({length: 3}).set('num'));
    } else if (abap.compare.eq(unique145, new abap.types.Character({length: 7}).set('boolean'))) {
  ...
  async traverse_object(INPUT) {
    ...
    for await (const unique148 of abap.statements.loop(lt_keys)) {
      lv_key.set(unique148);
...
class lcl_reader {
  ...
  async initialize() {
    ...
    for await (const unique149 of abap.statements.loop(lt_parsed)) {
      ls_parsed.set(unique149);
      let unique150 = ls_parsed.get().type;
      ...
      } else if (abap.compare.eq(unique150, abap.Classes['IF_SXML_NODE'].if_sxml_node$co_nt_value)) {
  ...
  async if_sxml_reader$next_node(INPUT) {
    ...
    let unique151 = this.if_sxml_reader$node_type;
