...
class cl_sql_statement {
  ...
  async execute_update(INPUT) {
    ...
    if (abap.compare.initial(lv_sql_message) === false) {
      const unique159 = await (new abap.Classes['CX_SQL_EXCEPTION']()).constructor_();
      unique159.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sql_statement.clas.abap","INTERNAL_LINE": 48};
      throw unique159;
    }
    try {
    ...
    if (abap.compare.initial(lv_sql_message) === false) {
      const unique160 = await (new abap.Classes['CX_SQL_EXCEPTION']()).constructor_();
      unique160.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sql_statement.clas.abap","INTERNAL_LINE": 57};
      throw unique160;
    }
  }
  async execute_query(INPUT) {
    ...
    if (abap.compare.initial(lv_sql_message) === false) {
      const unique161 = await (new abap.Classes['CX_SQL_EXCEPTION']()).constructor_();
      unique161.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sql_statement.clas.abap","INTERNAL_LINE": 71};
      throw unique161;
    }
    ...
    if (abap.compare.initial(lv_sql_message) === false) {
      const unique162 = await (new abap.Classes['CX_SQL_EXCEPTION']()).constructor_();
      unique162.EXTRA_CX = {"INTERNAL_FILENAME": "cl_sql_statement.clas.abap","INTERNAL_LINE": 84};
      throw unique162;
    }
    return result_set;
  }
