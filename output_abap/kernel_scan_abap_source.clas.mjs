...
class kernel_scan_abap_source {
  ...
  static async call(INPUT) {
    ...
    for await (const unique252 of abap.statements.loop(lt_stokesx)) {
      ls_stokesx.set(unique252);
  ...
  static async pass1(INPUT) {
    ...
    while (abap.compare.initial(source) === false) {
      abap.builtin.sy.get().index.set(unique253++);
  ...
  static async pass2(INPUT) {
    ...
    for await (const unique254 of abap.statements.loop(ct_statements)) {
      fs_ls_statement_.assign(unique254);
      ...
      for await (const unique255 of abap.statements.loop(ct_tokens,{from: fs_ls_statement_.get().from,to: fs_ls_statement_.get().to})) {
        ls_token.set(unique255);
      ...
      if (abap.compare.eq(contains_comment, abap.builtin.abap_true) && abap.compare.eq(contains_normal, abap.builtin.abap_true)) {
        ...
        for await (const unique256 of abap.statements.loop(ct_tokens,{from: fs_ls_statement_.get().from,to: fs_ls_statement_.get().to})) {
          ls_token.set(unique256);
          if (abap.compare.eq(ls_token.get().type, kernel_scan_abap_source.gc_token.get().comment)) {
        ...
        for await (const unique257 of abap.statements.loop(lt_delete)) {
          lv_index.set(unique257);
          await abap.statements.deleteInternal(ct_tokens,{index: lv_index});
        }
        for await (const unique258 of abap.statements.loop(lt_insert)) {
          ls_token.set(unique258);
