...
class cl_gdt_conversion {
  ...
  static async amount_outbound(INPUT) {
    ...
    let unique86 = im_currency_code;
    if (abap.compare.eq(unique86, new abap.types.Character(3).set('DKK')) || abap.compare.eq(unique86, new abap.types.Character(3).set('EUR')) || abap.compare.eq(unique86, new abap.types.Character(3).set('USD'))) {
      ex_value.set(im_value);
    } else if (abap.compare.eq(unique86, new abap.types.Character(3).set('VND'))) {
  ...
  static async language_code_inbound(INPUT) {
    ...
    let ex_value = new abap.types.Character(1, {"qualifiedName":"SPRAS"});
    if (INPUT && INPUT.ex_value) {ex_value = INPUT.ex_value;}
    abap.statements.assert(abap.compare.eq(new abap.types.Integer().set(1), new abap.types.Character(4).set('todo')));
  }
  ...
  static async unit_code_outbound(INPUT) {
    let im_value = new abap.types.Character(3, {"qualifiedName":"MSEHI"});
    if (INPUT && INPUT.im_value) {im_value.set(INPUT.im_value);}
  ...
  static async date_time_inbound(INPUT) {
    ...
    lv_str.set(im_value);
    abap.statements.replace({target: lv_str, all: true, with: new abap.types.Character(0).set(''), of: new abap.types.Character(1).set('-')});
    abap.statements.replace({target: lv_str, all: true, with: new abap.types.Character(0).set(''), of: new abap.types.Character(1).set(':')});
    abap.statements.replace({target: lv_str, all: true, with: new abap.types.Character(0).set(''), of: new abap.types.Character(1).set('T')});
    abap.statements.replace({target: lv_str, all: true, with: new abap.types.Character(0).set(''), of: new abap.types.Character(1).set('Z')});
  ...
  static async unit_code_inbound(INPUT) {
    let im_value = new abap.types.Character();
    if (INPUT && INPUT.im_value) {im_value = INPUT.im_value;}
    let ex_value = new abap.types.Character(3, {"qualifiedName":"MSEHI"});
    if (INPUT && INPUT.ex_value) {ex_value = INPUT.ex_value;}
    let unique87 = im_value;
    if (abap.compare.eq(unique87, new abap.types.Character(3).set('MTR'))) {
      ex_value.set(new abap.types.Character(1).set('M'));
    } else if (abap.compare.eq(unique87, new abap.types.Character(3).set('PCE'))) {
      ex_value.set(new abap.types.Character(2).set('PC'));
    } else if (abap.compare.eq(unique87, new abap.types.Character(3).set('KGM'))) {
      ex_value.set(new abap.types.Character(2).set('KG'));
    } else if (abap.compare.eq(unique87, new abap.types.Character(3).set('LTR'))) {
  ...
  static async language_code_outbound(INPUT) {
    let im_value = new abap.types.Character(1, {"qualifiedName":"SPRAS"});
    if (INPUT && INPUT.im_value) {im_value = INPUT.im_value;}
    let ex_value = new abap.types.Character(2, {"qualifiedName":"LAISO"});
    if (INPUT && INPUT.ex_value) {ex_value = INPUT.ex_value;}
    let unique88 = im_value;
    if (abap.compare.eq(unique88, new abap.types.Character(1).set('E'))) {
      ex_value.set(new abap.types.Character(2).set('en'));
    } else if (abap.compare.eq(unique88, new abap.types.Character(1).set('K'))) {
      ex_value.set(new abap.types.Character(2).set('da'));
    } else if (abap.compare.eq(unique88, new abap.types.Character(1).set('D'))) {
