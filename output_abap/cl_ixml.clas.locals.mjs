...
class lcl_named_node_map {
  ...
  async if_ixml_named_node_map$get_named_item_ns(INPUT) {
    ...
    for await (const unique191 of abap.statements.loop(this.mt_list)) {
      li_node.set(unique191);
...
class lcl_node {
  ...
  async if_ixml_element$find_from_name_ns(INPUT) {
    ...
    for await (const unique192 of abap.statements.loop(lt_nodes)) {
      li_top.set(unique192);
      ...
      let unique193 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique193++);
  ...
  async if_ixml_element$render(INPUT) {
    ...
    let unique194 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique194++);
      li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
      if (abap.compare.initial(li_node)) {
        break;
      }
  ...
  async if_ixml_node$get_depth() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique196 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique196++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
  ...
  async if_ixml_node$get_value() {
    ...
    if (abap.compare.eq((await this.mo_children.get().if_ixml_node_list$get_length()), abap.IntegerFactory.get(0))) {
      ...
      let unique197 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique197++);
        li_node.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_node)) {
          break;
        }
...
class lcl_parser {
  ...
  async if_ixml_parser$parse() {
    ...
    while (abap.compare.initial(lv_xml) === false) {
      abap.builtin.sy.get().index.set(unique198++);
  ...
  async parse_attributes(INPUT) {
    ...
    let unique199 = 1;
    while (true) {
      abap.builtin.sy.get().index.set(unique199++);
