...
class cl_abap_unit_assert {
  ...
  static async compare_tables(INPUT) {
    ...
    if (abap.compare.ne(abap.builtin.lines({val: act}), abap.builtin.lines({val: exp}))) {
      const unique296 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected table to contain ${abap.templateFormatting(abap.builtin.lines({val: exp}))} rows, got ${abap.templateFormatting(abap.builtin.lines({val: act}))}`)});
      unique296.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 162};
      throw unique296;
    }
    ...
    if (abap.compare.eq(type1.get().table_kind, abap.Classes['CL_ABAP_TABLEDESCR'].tablekind_hashed) || abap.compare.eq(type2.get().table_kind, abap.Classes['CL_ABAP_TABLEDESCR'].tablekind_hashed)) {
      for await (const unique297 of abap.statements.loop(fs_tab1_)) {
        fs_row1_.assign(unique297);
        lv_match.set(abap.builtin.abap_false);
        for await (const unique298 of abap.statements.loop(fs_tab2_)) {
          fs_row2_.assign(unique298);
          try {
        ...
        if (abap.compare.eq(lv_match, abap.builtin.abap_false)) {
          const unique299 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Hashed table contents differs`)});
          unique299.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 188};
          throw unique299;
        }
      }
    } else {
      const indexBackup1 = abap.builtin.sy.get().index.get();
      const unique300 = abap.builtin.lines({val: act}).get();
      for (let unique301 = 0; unique301 < unique300; unique301++) {
        abap.builtin.sy.get().index.set(unique301 + 1);
  ...
  static async assert_text_matches(INPUT) {
    ...
    if (abap.compare.eq(lv_match, abap.builtin.abap_false)) {
      const unique302 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({expected: pattern, actual: text, msg: msg});
      unique302.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 214};
      throw unique302;
    }
  }
  async abort(INPUT) {
  ...
  static async assert_bound(INPUT) {
    ...
    if (abap.compare.initial(act)) {
      const unique303 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected value to be bound`)});
      unique303.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 228};
      throw unique303;
    }
  }
  ...
  static async assert_not_bound(INPUT) {
    ...
    if (abap.compare.initial(act) === false) {
      const unique304 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected value to not be bound`)});
      unique304.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 236};
      throw unique304;
    }
  }
  ...
  static async assert_char_cp(INPUT) {
    ...
    if (abap.compare.np(act, exp)) {
      const unique305 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({expected: exp, actual: act, msg: msg});
      unique305.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 244};
      throw unique305;
    }
  }
  ...
  static async assert_char_np(INPUT) {
    ...
    if (abap.compare.cp(act, exp)) {
      const unique306 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Actual: ${abap.templateFormatting(act)}`)});
      unique306.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 254};
      throw unique306;
    }
  }
  async fail(INPUT) {
  ...
  static async fail(INPUT) {
    ...
    if (INPUT && INPUT.level) {level.set(INPUT.level);}
    let detail = INPUT?.detail || new abap.types.Character();
    const unique307 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: msg});
    unique307.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 261};
    throw unique307;
  }
  async skip(INPUT) {
  ...
  static async assert_differs(INPUT) {
    ...
    try {
      ...
      const unique308 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected different values`), actual: act, expected: exp});
      unique308.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 275};
      throw unique308;
    } catch (e) {
      if ((abap.Classes['KERNEL_CX_ASSERT'] && e instanceof abap.Classes['KERNEL_CX_ASSERT'])) {
        return;
  ...
  static async assert_true(INPUT) {
    ...
    if (abap.compare.ne(act, abap.builtin.abap_true)) {
      const unique309 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected abap_true`)});
      unique309.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 287};
      throw unique309;
    }
  }
  ...
  static async assert_false(INPUT) {
    ...
    if (abap.compare.ne(act, abap.builtin.abap_false)) {
      const unique310 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Expected abap_false`)});
      unique310.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 295};
      throw unique310;
    }
  }
  ...
  static async assert_equals(INPUT) {
    ...
    if (abap.compare.ca(type1, new abap.types.Character(11).set('CgyIFPDTXN8'))) {
      if (abap.compare.initial(type2) === false) {
        if (abap.compare.na(type2, new abap.types.Character(11).set('CgyIFPDTXN8'))) {
          const unique311 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Unexpected types`)});
          unique311.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 316};
          throw unique311;
        }
      }
    } else if (abap.compare.initial(type1) === false && abap.compare.initial(type2) === false) {
      if (abap.compare.ne(type1, type2)) {
        const unique312 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: new abap.types.String().set(`Unexpected types`)});
        unique312.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 324};
        throw unique312;
      }
    }
    if (abap.compare.eq(type1, new abap.types.Character(1).set('h'))) {
      ...
      if (abap.compare.ge(diff, tol)) {
        const unique313 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_();
        unique313.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 339};
        throw unique313;
      }
      ...
      const unique314 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: lv_msg, actual: lv_act, expected: lv_exp});
      unique314.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 353};
      throw unique314;
    }
  }
  ...
  static async assert_not_initial(INPUT) {
    ...
    if (abap.compare.initial(act)) {
      ...
      const unique315 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: lv_msg});
      unique315.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 368};
      throw unique315;
    }
  }
  ...
  static async assert_initial(INPUT) {
    ...
    if (abap.compare.initial(act) === false) {
      ...
      const unique316 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: lv_msg});
      unique316.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 381};
      throw unique316;
    }
  }
  ...
  static async assert_subrc(INPUT) {
    ...
    if (abap.compare.ne(act, exp)) {
      ...
      const unique317 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_({msg: lv_msg});
      unique317.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 394};
      throw unique317;
    }
  }
  ...
  static async assert_number_between(INPUT) {
    ...
    if (abap.compare.lt(number, lower) || abap.compare.gt(number, upper)) {
      const unique318 = await (new abap.Classes['KERNEL_CX_ASSERT']()).constructor_();
      unique318.EXTRA_CX = {"INTERNAL_FILENAME": "cl_abap_unit_assert.clas.abap","INTERNAL_LINE": 402};
      throw unique318;
    }
  }
}
