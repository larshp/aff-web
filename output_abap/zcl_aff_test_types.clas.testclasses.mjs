...
class ltcl_sanity {
  ...
  static INTERNAL_NAME = 'CLAS-ZCL_AFF_TEST_TYPES-LTCL_SANITY';
  static IMPLEMENTED_INTERFACES = [];
  static ATTRIBUTES = {};
  static METHODS = {"TEST1": {"visibility": "I", "parameters": {}}};
  constructor() {
  ...
  async test1() {
    ...
    let field = new abap.types.ABAPObject({qualifiedName: "CL_ABAP_DATADESCR", RTTIName: "\\CLASS=CL_ABAP_DATADESCR"});
    await abap.statements.cast(temp1, (await abap.Classes['CL_ABAP_TYPEDESCR'].describe_by_data({p_data: foo})));
    descr.set(temp1);
    await abap.Classes['CL_ABAP_UNIT_ASSERT'].assert_equals({act: (await descr.get().get_relative_name()), exp: new abap.types.Character(19).set('STRUC_WITH_NUM_TEXT')});
    await abap.Classes['CL_ABAP_UNIT_ASSERT'].assert_equals({act: descr.get().absolute_name, exp: new abap.types.Character(50).set('\\CLASS=ZCL_AFF_TEST_TYPES\\TYPE=STRUC_WITH_NUM_TEXT')});
    field.set((await descr.get().get_component_type({p_name: new abap.types.Character(15).set('NUMERICAL_TEXT1')})));
    await abap.Classes['CL_ABAP_UNIT_ASSERT'].assert_equals({act: (await field.get().get_relative_name()), exp: new abap.types.Character(8).set('NUM_TEXT')});
    await abap.Classes['CL_ABAP_UNIT_ASSERT'].assert_equals({act: field.get().absolute_name, exp: new abap.types.Character(39).set('\\CLASS=ZCL_AFF_TEST_TYPES\\TYPE=NUM_TEXT')});
  }
}
