...
class kernel_ixml_json_to_data {
  ...
  static async traverse(INPUT) {
    ...
    let unique235 = lo_type.get().kind;
    if (abap.compare.eq(unique235, abap.Classes['CL_ABAP_TYPEDESCR'].kind_struct)) {
      ...
      abap.builtin.sy.get().index.set(indexBackup1);
    } else if (abap.compare.eq(unique235, abap.Classes['CL_ABAP_TYPEDESCR'].kind_elem)) {
    ...
    } else if (abap.compare.eq(unique235, abap.Classes['CL_ABAP_TYPEDESCR'].kind_table)) {
      ...
      let unique237 = 1;
      while (true) {
        abap.builtin.sy.get().index.set(unique237++);
        li_child.set((await li_iterator.get().if_ixml_node_iterator$get_next()));
        if (abap.compare.initial(li_child)) {
          break;
        }
