...
let pattern = /INTERFACE (\w+) PUBLIC./m;
let abapObj = initialValue.match(pattern)?.[1];
let abapType = abapObj.replace("zif_aff_", "").substring(0, 4).toUpperCase();

const filename = "file:///"+abapObj;
...
async function abapChanged() {
  ...
  // @ts-ignore
  await abap.context.databaseConnections["DEFAULT"].execute(`UPDATE reposrc SET data = '${escape(contents)}' WHERE progname = '${abapObj.toUpperCase()}'`);
  console.dir("database updated changed");

  try {
    // @ts-ignore
    const result = await abap.Classes["CL_RUN"].run({object_type: new abap.types.String().set(abapType)});
